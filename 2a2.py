...
def loadIris():
	...
	trainingSet = np.zeros(shape=(75, 4))
	...
	trainingSet[0:25, 0:4] = feature_matrix[0:25, 0:4]
	trainingSet[25:50, 0:4] = feature_matrix[50:75, 0:4]
	trainingSet[50:75, 0:4] = feature_matrix[100:125, 0:4]

	return feature_matrix, labels, testSet, trainingSet
...
def minDistCl(feature_matrix, testSet, trainingSet):
	...
	for i in range(len(trainingSet)):
		e_dist1 = euclideanDistance(trainingSet[i], mean_vector1)
		e_dist2 = euclideanDistance(trainingSet[i], mean_vector2)
		e_dist3 = euclideanDistance(trainingSet[i], mean_vector3)
...
def linGausCl(feature_matrix, testSet, trainingSet):
	class1 = feature_matrix[0:25, 0:4]
	class2 = feature_matrix[50:75, 0:4]
	class3 = feature_matrix[100:125, 0:4]
	mean_vector1 = class1.mean(axis=0).T
	...
	for i in range(25):
		y_actual.append("Iris-virginica")
	for i in range(len(trainingSet)):
		m_dist1 = mahalanobisDistance(trainingSet[i], mean_vector1, S)		
		m_dist2 = mahalanobisDistance(trainingSet[i], mean_vector2, S)		
		m_dist3 = mahalanobisDistance(trainingSet[i], mean_vector3, S)		
...
def quadGausCl(feature_matrix, testSet, trainingSe)t:
	class1 = feature_matrix[0:25, 0:4]
	class2 = feature_matrix[50:75, 0:4]
	class3 = feature_matrix[100:125, 0:4]
	mean_vector1 = class1.mean(axis=0).T
	...
	for i in range(len(trainingSet)):
		g1 = np.dot(np.dot(trainingSet[i].T, W1), trainingSet[i]) + np.dot(w1.T, trainingSet[i]) + w1_0
		g2 = np.dot(np.dot(trainingSet[i].T, W2), trainingSet[i]) + np.dot(w2.T, trainingSet[i]) + w2_0
		g3 = np.dot(np.dot(trainingSet[i].T, W3), trainingSet[i]) + np.dot(w3.T, trainingSet[i]) + w3_0
...
def main():
	feature_matrix, labels, testSet, trainingSet = loadIris()
	minDistCl(feature_matrix, testSet, trainingSet)	
	linGausCl(feature_matrix, testSet, trainingSet)
	quadGausCl(feature_matrix, testSet, trainingSet)
